...
GetLatestTagFromGithub() {
    ...
    if [ ! "$version" ]; then
        ...
        version=$(echo "$tagInfo" | tr ',' '\n' | grep "\"name\":" | cut -d \" -f 4 | grep -v "beta" | head -n 1)
    fi
...
DownloadLatestFromGithub() {
    ...
    version=$(echo "$releaseInfo" | tr ',' '\n' | grep "\"tag_name\":" | cut -d \" -f 4 | grep -v "beta" | head -n 1)
    url=$(echo "$releaseInfo" | tr ',' '\n' | grep ".tar" | grep "\"browser_download_url\":" | grep -v "beta" | grep -v ".asc" | grep -v ".sha256" | cut -d \" -f 4 | head -n 1)
