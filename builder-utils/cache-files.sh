...
prepare_files_for_cache_de() {
	mkdir -p "$PORTEUX_BUILDER_PATH/caches"
...
prepare_files_for_cache() {
	...
	mkdir -p "$PORTEUX_BUILDER_PATH/caches/mime/packages"
	...
	mkdir -p "$PORTEUX_BUILDER_PATH/caches/applications"
	...
	mkdir -p "$PORTEUX_BUILDER_PATH/caches/schemas"
	...
	mkdir -p "$PORTEUX_BUILDER_PATH/caches/gdk-pixbuf-2.0/2.10.0/loaders"
...
generate_caches() {
	if [ -n "$(ls -A "$PORTEUX_BUILDER_PATH/caches/mime/packages" 2>/dev/null)" ]; then
	...
	if [ -n "$(ls -A "$PORTEUX_BUILDER_PATH/caches/applications" 2>/dev/null)" ]; then
		mkdir -p "$MODULE_PATH/packages/usr/share/applications"
	...
	if [ -n "$(ls -A "$PORTEUX_BUILDER_PATH/caches/schemas" 2>/dev/null)" ]; then
		mkdir -p "$MODULE_PATH/packages/usr/share/glib-2.0/schemas"
	...
	if [ -n "$(ls -A "$PORTEUX_BUILDER_PATH/caches/gdk-pixbuf-2.0/2.10.0/loaders" 2>/dev/null)" ]; then
		mkdir -p "$MODULE_PATH/packages/usr/lib$SYSTEM_BITS/gdk-pixbuf-2.0/2.10.0"
