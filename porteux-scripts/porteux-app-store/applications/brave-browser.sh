...
if [ `whoami` != root ]; then
    ...
    su -c "/opt/porteux-scripts/porteux-app-store/applications/brave-browser.sh $1 $2 $3"
    exit 0
fi
...
FULLVERSION=$(curl -s https://api.github.com/repos/brave/brave-browser/releases/latest | grep "\"tag_name\":" | cut -d \" -f 4 | head -n 1)
VERSION="${FULLVERSION//[vV]}"
APPLICATIONURL=https://github.com/brave/brave-browser/releases/download/v${VERSION}/brave-browser-${VERSION}-1.${ARCH}.rpm
...
/opt/porteux-scripts/porteux-app-store/module-builder.sh "${MODULEDIR}" "${OUTPUTDIR}/${CURRENTPACKAGE}-${VERSION}-${ARCH}.xzm" "${ACTIVATEMODULE}"
